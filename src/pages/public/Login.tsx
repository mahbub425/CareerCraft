...
import { ArrowRight, Eye, FileText, Info, LockKeyhole, Mail, ShieldCheck } from 'lucide-react';
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (currentUser && !currentUser.emailVerification) {
        ...
        setNotice('Your email is not verified yet. We sent a fresh verification link to your email.');
        return;
      }
      navigate(authService.isAdminUser(currentUser) ? '/admin/dashboard' : '/user/dashboard', { replace: true });
    } catch (err) {
      ...
      <main className="relative overflow-hidden pt-28">
        ...
        <div className="mx-auto grid max-w-7xl gap-10 px-5 py-12 sm:px-6 sm:py-16 lg:grid-cols-[0.9fr_1.1fr] lg:items-center lg:px-8 lg:py-20">
          <section className="mx-auto w-full max-w-xl rounded-3xl border border-blue-100 bg-white/90 p-6 shadow-2xl shadow-blue-900/10 backdrop-blur sm:p-8 lg:mx-0">
            ...
            <h1 className="mb-4 text-4xl font-black tracking-tight text-gray-950 sm:text-5xl">Welcome Back</h1>
            <p className="mb-8 max-w-lg text-lg leading-relaxed text-gray-600">
              Login to continue creating your CV with the selected template.
            </p>

            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                <label htmlFor="email" className="mb-2 block text-xs font-extrabold tracking-widest text-gray-700">
                  Email Address
                </label>
              ...
              <div>
                <label htmlFor="password" className="mb-2 block text-xs font-extrabold tracking-widest text-gray-700">
                  Password
                </label>
                <div className="flex items-center gap-3 rounded-xl border border-blue-100 bg-blue-50/60 px-4 py-4 text-gray-700 shadow-inner shadow-blue-900/5 transition-colors focus-within:border-blue-400 focus-within:bg-white focus-within:ring-4 focus-within:ring-blue-100">
              ...
              <div className="flex items-center justify-between gap-4 text-sm">
                <label className="flex items-center gap-3 font-medium text-gray-600">
                  <input type="checkbox" className="h-5 w-5 rounded border-blue-200 bg-white accent-blue-600" />
                  Remember me
                </label>
                <Link to="/login" className="font-bold text-blue-700 transition-colors hover:text-blue-900">
                  Forgot password?
                </Link>
              </div>
            ...
            <p className="mt-8 text-center text-sm font-medium text-gray-600">
              Don&apos;t have an account? <Link to="/register" className="font-extrabold text-blue-700 hover:text-blue-900">Start for free</Link>
            </p>
          </section>
          ...
          <section className="hidden lg:block">
            <div className="relative ml-auto max-w-2xl rounded-[2rem] border border-blue-100 bg-white/70 p-6 shadow-2xl shadow-blue-900/10 backdrop-blur">
              <div className="relative min-h-[520px] overflow-hidden rounded-3xl border border-blue-100 bg-blue-50">
                ...
                <div className="relative z-10 flex min-h-[520px] flex-col justify-end p-10">
                  ...
                  <h2 className="mb-5 max-w-xl text-4xl font-black leading-tight text-white drop-shadow">
                    Elevate Your Career with Professional Templates
                  </h2>
                  <p className="mb-8 max-w-xl text-xl leading-relaxed text-blue-50 drop-shadow">
                    Join over 50,000 professionals who built their dream career using CareerCraft&apos;s expert systems.
                  </p>
                  ...
                  <div className="group relative flex items-center gap-4 overflow-hidden rounded-2xl border border-white/35 bg-white/90 p-5 shadow-xl shadow-blue-950/20 backdrop-blur-md template-card-motion transition-all duration-500 hover:border-blue-200 hover:bg-white hover:shadow-2xl hover:shadow-blue-950/25">
                    ...
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-widest text-blue-500">Template Selected</p>
                      <p className="text-lg font-medium text-gray-950">Executive Modern v2.1</p>
                    </div>
                  </div>
                </div>
              </div>
